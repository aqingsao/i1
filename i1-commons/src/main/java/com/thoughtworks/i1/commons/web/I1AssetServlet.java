...
public class I1AssetServlet extends HttpServlet {
    public static final Logger logger = LoggerFactory.getLogger(I1AssetServlet.class);

    private String resourcePath;
    private Map<String, String> mimeExtensions = new HashMap();
    public static final String DEFAULT_MIME_TYPE = "text/html";

    public I1AssetServlet(String resourcePath) {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            URL resource = null;
            String contextPath = getContexPath(request, resourcePath);
            if(resourcePath.equals("index.html")){
                resource = getResource( resourcePath);
            } else {
                resource = getResource( resourcePath + request.getPathInfo());
            }
    ...
    private String getContexPath(HttpServletRequest request, String resourcePath) {
        String contextPath = request.getContextPath();
        if(contextPath.length() > 0 && contextPath.substring(contextPath.length() - 1).equals("/")){
    ...
    public I1AssetServlet setMimeExtensions(Map<String, String> mimeExtensions) {
